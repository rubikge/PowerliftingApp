    ...
    if widget == W_MAIN:
        ...
        for k, cell in enumerate(headersParticipants[1:]):
            ...
            if cell[0] in {'Р1', 'Р2', 'Р3'}:
                entries.append(Combobox(fDataP, values=['', 'Удачно', 'Неудачно', 'Отказ'], width=36))
                if sel != -1:
                    entries[k].current(data[sel][k+1])
                ...
                entries.append(Entry(fDataP, width=40))
                if sel != -1:
                    entries[k].insert(0, data[sel][k+1])
...
root.iconbitmap('pl.ico')
root.geometry(f'{w // 2}x{h // 2}+{w // 4}+{h // 8}')

# Картинка
canvas = Canvas(root, height=150, width=w // 2)
